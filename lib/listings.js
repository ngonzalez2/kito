...
/**
 * Normalizes filters to ensure no undefined or empty values are passed to SQL.
 */
    ...
    try {
      const result = await sql`
        ...
        WHERE status = 'approved'
          ...
          AND (${condition}::text IS NULL OR condition = ${condition}::text)
          AND (${location}::text IS NULL OR location = ${location}::text)
...
export async function createListing(listing) {
  const { title, description, price, condition, location, category, imageUrl } =
    listing;
